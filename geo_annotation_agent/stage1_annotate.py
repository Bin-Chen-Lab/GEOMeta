...
import pandas as pd
from .llm_client import AzureLLM
...
def run_stage1_raw_annotation(
    ...
    for ridx, row in df_input.iterrows():
        ...
        if expected_count is not None and expected_count > 0 and expected_gsm_ids and len(expected_gsm_ids) != expected_count:
            _reviewer_add_issue(
                reviewer=reviewer,
                gsm_id="GSE_LEVEL",
                gse_id=gse_id,
                ...
                severity="medium",
                message=(
                    f"Input GSM_Counts={expected_count} but extracted {len(expected_gsm_ids)} GSM IDs from chunk text."
                ),
                reviewer_action="manual_review",
            )
        ...
        for role_name, prompt_text in role_prompt_texts.items():
            try:
                ...
                role_rows_map[role_name] = [
                    {f: ("NA" if f not in {"GSM_ID", "GSE_ID"} else (gsm_id if f == "GSM_ID" else gse_id))
                     for f in ROLE_FIELDS[role_name]}
                ...
                _reviewer_add_issue(
                    ...
                    field_name=role_name,
                    severity="high",
                    message=f"Role {role_name} failed; emitted NA fallback rows. Error: {repr(e)}",
                    reviewer_action="manual_review",
                )
    ...
    if save_outputs:
        ...
        summary = {
            ...
            "stage1_unique_gsm": int(df_out["GSM_ID"].astype(str).nunique()) if not df_out.empty else 0,
            "stage1_dup_gsm": int(df_out["GSM_ID"].astype(str).duplicated().sum()) if not df_out.empty else 0,
