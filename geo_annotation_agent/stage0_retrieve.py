...
from .excel_safe import write_excel_with_long_text_parts
...
def split_gsm_records_by_metadata_limits(
    *,
    gsm_records: List[Dict[str, str]],
    gse_info: str,
    token_aware: bool,
    metadata_safe_token_limit: int,
    token_encoding: str,
    max_gsms_per_chunk: int = 0,
) -> List[Dict[str, Any]]:
    """
    Split GSM records into Stage 0 metadata chunks using metadata-size limits,
    not a fixed 30/300 GSM batch size.

    Policy:
      - Small GSEs stay together in one chunk.
      - Large GSEs are split only as needed to keep the estimated metadata token
        size below `metadata_safe_token_limit`.
      - Splitting is always between complete GSM records.
      - Excel cell limits are handled separately by the Excel-safe writer via
        GSE_Info_Part_* and GSM_Info_Part_* columns; Excel is not the
        authoritative storage format.
      - `max_gsms_per_chunk` is an optional emergency cap only. Set to 0 or a
        negative value to disable any fixed GSM-count cap.
    """
    if not gsm_records:
        return []

    metadata_safe_token_limit = max(1000, int(metadata_safe_token_limit))
    max_gsms_per_chunk = int(max_gsms_per_chunk or 0)

    chunks: List[Dict[str, Any]] = []
    current: List[Dict[str, str]] = []

    def metadata_tokens(records: List[Dict[str, str]]) -> int:
        gsm_text = build_chunked_gsm_text(records)
        if not token_aware:
            return 0
        return int(
            estimate_tokens(
        ...
        chunks.append(
            {
                "records": current,
                "chunk_reason": reason,
                "metadata_tokens": metadata_tokens(current),
            }
        )
        current = []
    ...
    for rec in gsm_records:
        ...
        too_many_gsms = max_gsms_per_chunk > 0 and len(candidate) > max_gsms_per_chunk
        candidate_tokens = metadata_tokens(candidate)
        too_many_tokens = bool(token_aware and candidate_tokens > metadata_safe_token_limit)

        if too_many_gsms or too_many_tokens:
            if too_many_tokens:
                flush("split_by_stage0_metadata_token_limit")
            else:
                flush("split_by_optional_max_gsms_per_chunk")
            current = [rec]
        else:
            current = candidate
    ...
    # If a single-GSM chunk itself exceeds the metadata token limit, keep it intact
    # but flag it. Stage 1 performs the final prompt-aware token check before LLM calls.
    for ch in chunks:
        ch["single_gsm_over_stage0_token_limit"] = bool(
            len(ch["records"]) == 1
            and token_aware
            and int(ch["metadata_tokens"]) > metadata_safe_token_limit
        )
...
def run_stage0_retrieval(
                ...
                rows.append(
                    {
                        ...
                        "Failed_GSM_Count": failed_gsms_for_this_gse,
                        "GSE_Info_Chars": len(str(gse_info)),
                        "GSM_Info_Chars": len(str(gsm_info_chunk)),
                        "Stage0_Metadata_Estimated_Tokens": int(chunk_obj.get("metadata_tokens", 0)),
    ...
    wanted_cols = [
        ...
        "GSE_Info_Chars",
        "GSM_Info_Chars",
        "Stage0_Metadata_Estimated_Tokens",
    ...
    out_jsonl = Path(cfg.outputs_dir) / f"{cfg.run_version}_stage0_input.full.jsonl"

    # Full-fidelity authoritative outputs. These preserve complete GSE_Info/GSM_Info.
    # Do not use Excel as the authoritative restart artifact for long metadata.
    df_stage0.to_parquet(out_parq, index=False)

    if bool(getattr(cfg, "stage0_save_full_metadata_jsonl", True)):
        df_stage0.to_json(
            out_jsonl,
            orient="records",
            lines=True,
    ...
    # Excel review output. Long GSE_Info/GSM_Info are split into ordered part columns
    # to avoid Excel's 32,767-character per-cell limit. The preview columns are safe;
    # the complete values are also present in GSE_Info_Part_* / GSM_Info_Part_* columns.
    ...
    summary = {
        ...
        "excel_long_text_part_limit": int(getattr(cfg, "stage0_excel_text_part_char_limit", 32000)),
        "stage0_token_aware_chunking": bool(getattr(cfg, "stage0_token_aware_chunking", True)),
    ...
    print("[SAVED] Stage0 input Excel review copy:", out_xlsx)
    print("[SAVED] Stage0 input Parquet full:", out_parq)
    if bool(getattr(cfg, "stage0_save_full_metadata_jsonl", True)):
        print("[SAVED] Stage0 input JSONL full:", out_jsonl)
