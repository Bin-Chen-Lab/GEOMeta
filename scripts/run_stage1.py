...
def main():
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Optional input override (.xlsx/.xls/.csv/.tsv/.parquet). "
            "If omitted, config.input_xlsx is used. "
            "This can be either a manually prepared Stage1-style input table "
            "or a Stage0 output file. Prefer Stage0 parquet for full metadata."
        ),
    )
    ap.add_argument(
        "--run-version",
    ...
    suffix = Path(cfg.input_xlsx).suffix.lower()
    if suffix in {".xlsx", ".xls"}:
        df_input = pd.read_excel(cfg.input_xlsx, engine="openpyxl")
    elif suffix == ".parquet":
        df_input = pd.read_parquet(cfg.input_xlsx)
    elif suffix == ".csv":
        df_input = pd.read_csv(cfg.input_xlsx)
    elif suffix == ".tsv":
        df_input = pd.read_csv(cfg.input_xlsx, sep="\t")
    else:
        raise ValueError(f"Unsupported Stage1 input file type: {cfg.input_xlsx}")
