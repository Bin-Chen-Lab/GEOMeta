Agent task: Timepoint standardization

Standardize the `Timepoint` field according to the rules below. Preserve the input order and return one standardized value for each original term.
...
1. Normalize Nonstandard Values Using Predefined Timepoint Logic:
...
9). Terms Involving 'DIV':
  Convert 'DIV' to 'Days In Vitro', e.g., '14 DIV' → '14 Days In Vitro'.
...
25). Pre-/Post- Terms:
If `ATT` is explicitly defined as treatment in the available context, `-1 Days from ATT` → `Pre-treatment Day: 1`. Otherwise, do not expand an undefined abbreviation by assumption.
