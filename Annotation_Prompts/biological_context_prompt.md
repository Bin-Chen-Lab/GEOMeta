...
- Unclear or Multiple Possible Diseases: annotate as ‘NA’. Use NA only when disease-related information is present but the correct disease assignment cannot be determined because the information is ambiguous, conflicting, or insufficient to distinguish among multiple possible diseases. Do not use NA when disease information is simply absent; in those cases annotate as ‘No Disease Mentioned’.

2). Additional decision rules for Disease annotation: 
...
Keywords: 'healthy control,' 'normal tissue,' 'control only when explicitly healthy/non-diseased'.
...
Examples: MDA-MB-231 → Breast Cancer (triple-negative)(Inferred).
PLC/PRF/5 → Hepatocellular Carcinoma (Inferred).
DLD-1 → Colorectal Cancer (Inferred).
...
- Example: PLC/PRF/5 → Hepatocellular Carcinoma (Inferred).
...
- Specific Disease: Explicitly mentions disease or known diseased origin.
...
- Classification: Breast Cancer (triple-negative) (Inferred).
...
- Classification: Hepatocellular carcinoma (Inferred).
...
2). GSM12306: Tissue: Colon adenocarcinoma. Disease state: Colorectal cancer → Colon Adenocarcinoma (Extracted).
