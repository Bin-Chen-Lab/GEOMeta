Agent task: Disease mapping

Map each standardized `Disease` term to the most appropriate CTD/MEDIC disease concept using the candidate records supplied by the Python mapping workflow.

## General Requirements

- Preserve one mapping result for each input disease term.
- Select mappings only from the supplied CTD/MEDIC candidate context.
- Return `NA` when no candidate is sufficiently supported.
- Do not invent DiseaseName or DiseaseID values.
...
If the input term matches a candidate `DiseaseName` or listed synonym after normalization, return the corresponding canonical `DiseaseName` and `DiseaseID`. Matching should be case-insensitive and robust to minor punctuation or spacing differences.

3. Multi-Disease Terms
If the input contains multiple disease components joined by `+`, treat each component independently.
Return matched disease names in the same order, joined by ` + `. If no suitable match is found for one component, return `NA` for that component.
...
If the subtype or modifier is clinically central and explicitly stated in the input, preserve it whenever supported by the ontology. If the subtype is not available or not clearly supported, return the most appropriate general disease concept that preserves the core disease identity. Do not return numbered or named subtypes unless the same subtype identifier appears in the input.
Examples:
...
## Output Requirements
- Return exactly one mapping for each input disease term.
- Preserve the standardized input disease term in `raw`.
- Return the selected canonical CTD/MEDIC `DiseaseName` and corresponding
  `DiseaseID`.
- Return `NA` for both `DiseaseName` and `DiseaseID` when no supplied candidate
  is sufficiently supported.
- Provide a brief justification in `explanation`.
- Do not invent `DiseaseName` or `DiseaseID` values.
