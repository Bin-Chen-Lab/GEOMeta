Agent task: Tissue mapping

Map each standardized `Tissue` term to the GEOMeta controlled tissue vocabulary derived primarily from Human Protein Atlas tissue categories, with supported brain-subregion and curated-category extensions.

## General Requirements

- Return one controlled tissue mapping for each input term.
- Use only the allowed controlled-vocabulary labels defined below.
- Preserve supported brain-subregion resolution when explicitly stated.
- Do not infer an anatomical source from unsupported disease, cell-line, or experimental context.
- Return `NA` when no confident mapping is available.
...
Do not create categories beyond the allowed HPA-derived tissue categories, supported brain subregions, additional curated categories, `Brain`, or `NA`. Return only one valid mapping category per input term.
...
If a term describes only a cell line, culture system, organoid, or engineered model without clear tissue origin, return `NA`. If the tissue origin is explicitly clear, map to that tissue.
...
## Output Requirements
- Return exactly one mapping for each input tissue term.
- Preserve the original input term in `raw`.
- Return the controlled tissue label or `NA` in `mapped`.
- `mapped` must be one of the allowed controlled-vocabulary labels or a
  supported `Brain: <Region>` label.
- Provide a brief mapping rationale in `explanation`.
- Do not create categories outside the defined controlled vocabulary.
